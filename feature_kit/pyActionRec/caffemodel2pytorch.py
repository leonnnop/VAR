...
class Net(nn.Module):
	def __init__(self, prototxt, *args, **kwargs):
		...
		for layer in list(self.net_param.layer) + list(self.net_param.layers):
			...
			if isinstance(layer_type, int):
				print('Loading int layer type [{}]'.format(layer_type))
				layer_type = layer.LayerType.Name(layer_type)

			if layer_type not in ['BN','Convolution','Pooling','ReLU','Eltwise','InnerProduct', 'Dropout',\
				'Flatten','Concat']: print(layer_type)
			...
			if module_constructor is not None:
				# heck bcz torch>0.4 can not have module name with '.'
				...
				module = module_constructor(layer.name, param)

				self.add_module(layer_name, module if isinstance(module, nn.Module) else CaffePythonLayerModule(module, caffe_input_variable_names, caffe_output_variable_names, param.get('param_str', '')) if type(module).__name__.endswith('Layer') else FunctionModule(module))

				module = getattr(self, layer_name)
				module.caffe_layer_name = layer_name
	...
	def forward(self, data = None, out = [], **variables):
		if data is not None:
			variables['data'] = data
		...
		caffe_output_variable_names.add(*out)
	...
	def copy_from(self, weights):
		try:
			...
			for layer in list(self.net_param.layer) + list(self.net_param.layers):
				...
				if module is None:
					print('[LOAD] skip [{}] with type [{}]'.format(layer_name, layer.type))
					continue

				if layer.type == 'BN':
					parameters = {name : convert_to_gpu_if_enabled(torch.FloatTensor(blob.data)).view(list(blob.shape.dim) if len(blob.shape.dim) > 0 else [blob.num, blob.channels, blob.height, blob.width]) for name, blob in zip(['weight', 'bias', 'running_mean', 'running_var'], layer.blobs)}
					# breakpoint()
				else: # for conv2d 
...
modules = dict(
	BN = lambda name, param: BatchNormalization(name, param),
	Flatten = lambda name, param: Flatten(name, param),
	Concat = lambda name, param: Concat(name, param),
	# 
	Convolution = lambda name, param: Convolution(name, param),
	InnerProduct = lambda name, param: InnerProduct(name, param),
	Softmax = lambda name, param: nn.Softmax(dim = param.get('axis', -1)),
	ReLU = lambda name, param: nn.ReLU(),
	Dropout = lambda name, param: nn.Dropout(p = param['dropout_ratio'], inplace=True),
	# Fix modified pooling and check eltwise works fine
	Pooling = lambda name, param: Pooling(name, param),
	Eltwise = lambda name, param: [torch.mul, torch.add, torch.max][param.get('operation', 1)],
)
...
class Convolution(nn.Conv2d):
	def __init__(self, name, param):
		# breakpoint()
		assert not('kernel_h' in param and 'kernel_size' in param)
		assert not('stride_h' in param and 'stride' in param)
		assert not('pad_h' in param and 'pad' in param)

		if 'kernel_h' in param or 'stride_h' in param or 'pad_h' in param:
			kernel_size = (first_or(param, 'kernel_h', 1),first_or(param, 'kernel_w', 1))
			stride = (first_or(param, 'stride_h', 1),first_or(param, 'stride_w', 1))
			padding = (first_or(param, 'pad_h', 0),first_or(param, 'pad_w', 0))
			dilation = (first_or(param, 'dilation_h', 1),first_or(param, 'dilation_w', 1))
		elif 'kernel_size' in param or 'stride' in param or 'pad' in param:
		...
		# heck bcz caffe do not preserve in_channel
		# keep in mind: set_parameter will set in_channel
		...
		self.bias_term = first_or(param, 'bias_term', True)

	def forward(self, x):
		if self.weight.numel() == 0 and self.bias.numel() == 0:
			requires_grad = [self.weight.requires_grad, self.bias.requires_grad]
			super(Convolution, self).__init__(x.size(1), self.out_channels, kernel_size = self.kernel_size, stride = self.stride, padding = self.padding, dilation = self.dilation, groups=self.groups, bias=self.bias_term)
			if not self.bias_term: self.bias = None
			convert_to_gpu_if_enabled(self)
...
class BatchNormalization(nn.BatchNorm2d):
	def __init__(self, name, param):
		# param: slope_filler and bias_filler, not that matter in my case; only eval mode is allowed
		super(BatchNormalization, self).__init__(1)
		self._name = name

	def forward(self, x):
		assert not self.training
		return super(BatchNormalization, self).forward(x)

	def set_parameters(self, weight=None, bias=None, running_mean=None, running_var=None):
		...
		self.running_mean = nn.Parameter(running_mean.type_as(self.running_mean), requires_grad = self.running_mean.requires_grad)
		self.running_var = nn.Parameter(running_var.type_as(self.running_var), requires_grad = self.running_var.requires_grad)

		self.num_features = self.weight.size(1)
...
class Concat(nn.Module):
	def __init__(self, name, param):
		...
		self.dim = first_or(param, 'axis', 1)
		self._name = name
...
class Add(nn.Module):
	def __init__(self, name, param):
		...
		if 'coeff' in param: breakpoint()
		self.coeffs = first_or(param, 'coeff', [1,1])
		self._name = name

	def forward(self, x1, x2):
		return torch.add(x1*self.coeffs[0], x2*self.coeffs[1])

class Pooling(nn.Module):
	def __init__(self, name, param):
		super(Pooling, self).__init__()
		self._pool_func = [nn.functional.max_pool2d, nn.functional.avg_pool2d][param['pool']]
		...
		if 'kernel_h' in param or 'stride_h' in param or 'pad_h' in param:
			self.kernel_size = (first_or(param, 'kernel_h', 1),first_or(param, 'kernel_w', 1))
			self.stride = (first_or(param, 'stride_h', 1),first_or(param, 'stride_w', 1))
			self.padding = (first_or(param, 'pad_h', 0),first_or(param, 'pad_w', 0))
		elif 'kernel_size' in param or 'stride' in param or 'pad' in param:
			self.kernel_size = first_or(param, 'kernel_size', 1)
			self.stride = first_or(param, 'stride', 1)
			self.padding = first_or(param, 'pad', 0)
		else:
			raise ValueError

		self._global_pool = first_or(param, 'global_pooling', False)
		self._name = name

	def forward(self, x):

		if self._global_pool:
			self.kernel_size = (int(x.shape[-2]),int(x.shape[-1]))
		out = self._pool_func(
					x, 
					self.kernel_size, self.stride, self.padding, 
					# caffe, by default, use ceil in pooling
					ceil_mode=True)
